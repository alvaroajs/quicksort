...
class FilaEstatica:
    """Implementação de fila estática usando lista Python com tamanho máximo."""
    
    def __init__(self, tamanho_maximo):
        self.tamanho_maximo = tamanho_maximo
        self.fila = [None] * tamanho_maximo  # Array com tamanho fixo
        self.inicio = 0
        self.fim = 0
        self.tamanho = 0
    ...
    def enfileirar(self, item):
        ...
        if self.esta_cheia():
            raise OverflowError("Fila cheia! Não é possível enfileirar mais elementos.")
        
        self.fila[self.fim] = item
        self.fim = (self.fim + 1) % self.tamanho_maximo  # Circular
        self.tamanho += 1
    ...
    def desenfileirar(self):
        ...
        self.fila[self.inicio] = None  # Limpa a referência
        self.inicio = (self.inicio + 1) % self.tamanho_maximo  # Circular
        self.tamanho -= 1
        return item
...
if __name__ == "__main__":
    ...
    try:
        # ETAPA 1: Lê dados do CSV e enfileira em fila estática
        ...
        if movie_data:
            ...
            # Cria fila estática e enfileira os dados
            fila_inicial = FilaEstatica(len(movie_data))
            for linha in movie_data:
                fila_inicial.enfileirar(linha)
            
            # ETAPA 2: Desenfileira para vetor estático e ordena
            vetor_estatico = []
            while not fila_inicial.esta_vazia():
                vetor_estatico.append(fila_inicial.desenfileirar())
            ...
            fila_final = FilaEstatica(len(vetor_estatico))
            ...
            temp_fila = FilaEstatica(len(vetor_estatico))
            
            # Desenfileira tudo para mostrar e guarda em fila temporária
            while not fila_final.esta_vazia():
                item = fila_final.desenfileirar()
                resultado_para_mostrar.append(item)
                temp_fila.enfileirar(item)
